...
/// Port inside the container (must match mosquitto.conf).
const CONTAINER_TLS_PORT: u16 = 8883;
/// Determined at runtime via `testcontainers` `get_host()` so that tests work
/// both on bare metal and inside a Docker-in-Docker (DinD) setup.
const FALLBACK_BROKER_HOST: &str = "localhost";
...
fn generate_test_certs() -> TestCerts {
    // Use a directory visible to the Docker daemon. When running inside a
    // container the default /tmp lives on the overlay FS and is invisible to
    // sibling containers started via the host's Docker socket.  The project
    // directory is bind-mounted from the host, so temp dirs created there are
    // accessible for Docker bind mounts.
    ...
    server_params.subject_alt_names = vec![
        ...
        // Docker gateway IPs used when running inside DinD
        SanType::IpAddress(IpAddr::V4(Ipv4Addr::new(172, 17, 0, 1))),
        SanType::IpAddress(IpAddr::V4(Ipv4Addr::new(172, 18, 0, 1))),
    ];
    ...
    // Admin cert (CN = admin, full topic access)
    ...
    let admin_pem = admin_cert.pem().into_bytes();
    let admin_key_pem = admin_key.serialize_pem().into_bytes();

    std::fs::write(certs_path.join("admin.pem"), &admin_pem).unwrap();
    std::fs::write(certs_path.join("admin.key"), &admin_key_pem).unwrap();
...
";
    ...
    let acl = "\
user test-drone-01
topic readwrite test/nodes/test-drone-01/#

user admin
topic readwrite #
";
    std::fs::write(certs_path.join("acl"), acl).unwrap();
...
impl MosquittoContainer {
    ...
    fn admin_tls_config(&self) -> TlsConfiguration {
        TlsConfiguration::Simple {
            ca: self._certs.ca_pem.clone(),
            alpn: None,
            client_auth: Some((
    ...
    fn make_admin_tls_client(&self, client_id: &str) -> (AsyncClient, rumqttc::EventLoop) {
        let mut opts = MqttOptions::new(client_id, &self.broker_host, self.tls_port);
        opts.set_keep_alive(Duration::from_secs(10));
        opts.set_transport(Transport::tls_with_config(self.admin_tls_config()));
...
async fn start_mosquitto() -> MosquittoContainer {
    ...
    let container = image
        ...
        .with_mount(Mount::bind_mount(
            certs_path.join("acl").to_str().unwrap(),
    ...
    let broker_host = container
        .get_host()
        .await
        .map(|h| h.to_string())
        .unwrap_or_else(|_| FALLBACK_BROKER_HOST.to_string());
...
async fn test_telemetry_publish_subscribe() {
    ...
    // Subscriber client (admin — server monitoring drone telemetry)
    let (sub_client, mut sub_eventloop) = broker.make_admin_tls_client("integ-telem-sub");
...
async fn test_command_lifecycle_status_transitions() {
    ...
    // "Server" client (admin): subscribes to status + result, publishes command on /in
    let (server_client, mut server_eventloop) = broker.make_admin_tls_client("integ-cmd-server");
    ...
    let in_progress =
        json!({"state": "in_progress", "uuid": "cmd-1234", "ts": Utc::now().to_rfc3339()});
    drone_client
        .publish(
            &status_topic,
            QoS::AtLeastOnce,
    ...
    let completed =
        json!({"state": "completed", "uuid": "cmd-1234", "ts": Utc::now().to_rfc3339()});
    drone_client
        .publish(
            &status_topic,
            QoS::AtLeastOnce,
...
async fn test_expired_command_topic_routing() {
    ...
    // Subscriber watches status topic (admin — server monitoring drone)
    let (sub_client, mut sub_eventloop) = broker.make_admin_tls_client("integ-expired-sub");
    wait_for_connect(&mut sub_eventloop, "sub").await;
    sub_client
        .subscribe(&status_topic, QoS::AtLeastOnce)
        .await
    ...
    // Publisher sends expired command on /in and also the expected expired status (drone)
    ...
    let expired_status =
        json!({"state": "expired", "uuid": "expired-cmd-001", "ts": Utc::now().to_rfc3339()});
    pub_client
        .publish(
            &status_topic,
            QoS::AtLeastOnce,
...
async fn test_rejected_command_topic_routing() {
    ...
    // Subscriber (admin — server monitoring drone)
    let (sub_client, mut sub_eventloop) = broker.make_admin_tls_client("integ-rejected-sub");
    wait_for_connect(&mut sub_eventloop, "sub").await;
    sub_client
        .subscribe(&status_topic, QoS::AtLeastOnce)
        .await
    ...
    let rejected_status =
        json!({"state": "rejected", "uuid": "rejected-cmd-001", "ts": Utc::now().to_rfc3339()});
    pub_client
        .publish(
            &status_topic,
            QoS::AtLeastOnce,
...
async fn test_sequential_publish_delivery() {
    ...
    // Use admin TLS clients for sequential publish testing
    let (sub_client, mut sub_eventloop) = broker.make_admin_tls_client("integ-reconn-sub");
    wait_for_connect(&mut sub_eventloop, "sub").await;
    sub_client.subscribe(topic, QoS::AtLeastOnce).await.unwrap();
    wait_for_suback(&mut sub_eventloop, "reconn-sub").await;

    let (pub_client, mut pub_eventloop) = broker.make_admin_tls_client("integ-reconn-pub");
    wait_for_connect(&mut pub_eventloop, "pub").await;
...
async fn test_wildcard_command_subscription() {
    ...
    // Subscriber (drone — listens for incoming commands on its own topics)
    ...
    // Publisher (admin — server sending commands to drone)
    let (pub_client, mut pub_eventloop) = broker.make_admin_tls_client("integ-wildcard-pub");
    wait_for_connect(&mut pub_eventloop, "pub").await;
